...
await mkdir('dist/linb-kanban', { recursive: true });
await mkdir('dist/preview', { recursive: true });
await build({ entryPoints: ['src/main.ts'], bundle: true, outfile: 'dist/linb-kanban/main.js', platform: 'browser', format: 'cjs', target: 'es2020', external: ['obsidian'], minify: true, logLevel: 'info' });
for (const file of ['manifest.json', 'styles.css']) await copyFile(file, `dist/linb-kanban/${file}`);
...
const code = await readFile('dist/linb-kanban/main.js', 'utf8');
