...
test('repository serializes simultaneous saves to one file without losing either change', async () => {
  ...
  const first = repository.save('board.moss', { type: 'board:update', patch: { title: '更新标题' } });
  const second = repository.save('board.moss', { type: 'board:update', patch: { description: '保留新的描述' } });
  const third = repository.save('board.moss', { type: 'card:add', card: createCard(initial.columns[0].id, { title: '第三次保存' }) });
...
test('repository applies edits to current disk content and retains unrelated external changes', async () => {
  ...
  const cached = await repository.read('board.moss');
  ...
  const saved = await repository.save('board.moss', { type: 'card:update', id: first.id, patch: { body: '本地更新' }, expectedUpdatedAt: cached.cards[0].updatedAt });
...
test('repository rejects stale card edits against external changes and keeps newer disk content', async () => {
  ...
  await assert.rejects(repository.save('board.moss', { type: 'card:update', id: card.id, patch: { title: '过期修改' }, expectedUpdatedAt: card.updatedAt }), /其他窗口修改/);
...
test('a failed write rejects its caller but does not poison later queued saves', async () => {
  ...
  const repository = new BoardRepository(io);
  const failed = repository.save('board.moss', { type: 'board:update', patch: { title: '不能落盘的标题' } });
  const recovered = repository.save('board.moss', { type: 'board:update', patch: { description: '后续保存成功' } });
...
test('corrupt or newer-version board files are never overwritten by a save', async () => {
  ...
  for (const original of ['{ damaged and irreplaceable content', JSON.stringify({ ...initial, version: 99 })]) {
    ...
    await assert.rejects(repository.read('board.moss'));
    await assert.rejects(repository.save('board.moss', { type: 'board:update', patch: { title: '不可覆盖' } }));
    assert.equal(store.read(), original);
    assert.equal(store.writes(), 0);
  }
  store.writeExternal(serializeBoard(initial));
  const recovered = await repository.save('board.moss', { type: 'board:update', patch: { title: '修复文件后可再次保存' } });
  assert.equal(recovered.title, '修复文件后可再次保存');
  assert.equal(store.writes(), 1);
});

test('different board files use independent save queues', async () => {
  const disk = new Map([['one.moss', serializeBoard(createBoard('一'))], ['two.moss', serializeBoard(createBoard('二'))]]);
  let releaseFirst: () => void = () => {};
  const firstGate = new Promise<void>((resolve) => { releaseFirst = resolve; });
  const io: BoardIO = {
    async read(path) { return disk.get(path)!; },
    async process(path, update) {
      if (path === 'one.moss') await firstGate;
  ...
  const repository = new BoardRepository(io);
  const first = repository.save('one.moss', { type: 'board:update', patch: { title: '一已保存' } });
  const second = await repository.save('two.moss', { type: 'board:update', patch: { title: '二已保存' } });
  assert.equal(second.title, '二已保存');
  assert.equal(parseBoard(disk.get('one.moss')!).title, '一');
  releaseFirst();
